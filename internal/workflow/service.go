...
func New(repo domain.Repository, clock Clock) *Service {
	...
	return &Service{repo: repo, clock: clock, ids: rand.New(rand.NewSource(time.Now().UnixNano()))}
}
...
func (s *Service) Create(ctx context.Context, env Envelope, input CreateInput) (domain.CommitResult, error) {
	...
	now := s.clock().UTC()
	c, err := domain.NewCase(s.newCaseID(), input.CableSegment, input.WorkScope, input.WindowStart, input.WindowEnd, input.Coordinator, input.RiskBaseline, now)
	if err != nil {
		return domain.CommitResult{}, err
	}
...
func (s *Service) apply(c *domain.MaintenanceCase, action string, env Envelope, now time.Time) error {
	...
	case "put_evidence":
		...
		return c.PutEvidence(domain.ReadinessEvidence{ID: s.newEntityID("evidence"), Category: in.Category, Subject: in.Subject, Reference: in.Reference, ValidUntil: in.ValidUntil, Note: in.Note}, env.Actor, now)
	...
	case "acknowledge":
		...
		return c.Acknowledge(domain.CoordinationAcknowledgement{ID: s.newEntityID("ack"), PartyRole: in.PartyRole, Decision: in.Decision, Comment: in.Comment}, env.Actor, now)
	...
	case "add_progress":
		...
		return c.AddProgress(s.newEntityID("progress"), in.Text, env.Actor, now)
	case "add_deviation":
		var in DeviationInput
		if err := decodePayload(env.Payload, &in); err != nil {
			return err
		}
		return c.AddDeviation(domain.OperationalDeviation{ID: s.newEntityID("deviation"), Severity: in.Severity, Description: in.Description, ImmediateAction: in.ImmediateAction}, env.Actor, now)
